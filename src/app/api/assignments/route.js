...
export async function POST(req) {
    try {
      ...
      const caregiverResult = await pool.query("SELECT * FROM users WHERE id = $1 AND role = $2", [
        caregiver_id,
        UserRole.CAREGIVER,
      ])
  
      if (caregiverResult.rows.length === 0) {
        return NextResponse.json({ error: "Caregiver tidak ditemukan" }, { status: 400 })
      }
      ...
      if (patientResult.rows.length === 0) {
        return NextResponse.json({ error: "Pasien tidak ditemukan" }, { status: 400 })
      }
